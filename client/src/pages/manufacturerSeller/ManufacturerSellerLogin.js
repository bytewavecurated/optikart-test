...
const ManufacturerSellerLogin = () => {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);
  const [loading, setLoading] = useState(false);
  const { manufacturerSellerLogin } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await manufacturerSellerLogin(email, password);

    if (result.success) {
      if (result.requiresOTP) {
        navigate('/manufacturer-seller/verify-otp', { state: { sellerId: result.sellerId, email } });
      } else {
        navigate('/manufacturer-seller/dashboard');
      }
    } else {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' }}>
      <div style={{ background: '#fff', borderRadius: '12px', boxShadow: '0 10px 40px rgba(0,0,0,0.1)', width: '100%', maxWidth: '400px', padding: '40px' }}>
        <h1 style={{ fontSize: '28px', fontWeight: '700', marginBottom: '8px', color: '#212121' }}>Manufacturer Seller Login</h1>
        <p style={{ color: '#757575', marginBottom: '32px' }}>Access your seller dashboard</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            ...
            style={{
              width: '100%',
              padding: '14px',
              background: loading ? '#9e9e9e' : '#4facfe',
              color: '#fff',
              border: 'none',
              borderRadius: '8px',
              fontSize: '16px',
          ...
          <div style={{ textAlign: 'center' }}>
            <Link to="/" style={{ color: '#4facfe', fontSize: '14px', textDecoration: 'none' }}>
              Back to Home
            </Link>
          </div>
        </form>
...
export default ManufacturerSellerLogin;
